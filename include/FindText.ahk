...
IfNotEqual, A_LineFile, %A_ScriptFullPath%, Goto, ft_End
...
{
  ListLines, Off
  if (A_Gui="ft_Main") and (A_GuiControl="ft_scr")
...
s := ASCII(s)
GuiControl, ft_Main:, ft_MyPic, % Trim(s,"`n")
return
...
Gui, Destroy
WinWaitClose
ft_cors:=ft_getc(px,py,ft_ww,ft_hh)
...
GuiControl,, ft_SelCol
GuiControl,, ft_Gray
GuiControl,, ft_Modify, % ft_Modify:=0
GuiControl,, ft_UsePos, % ft_UsePos:=0
GuiControl, Focus, ft_Gray
Gosub, ft_Reset
Gui, Show, Center
DetectHiddenWindows, Off
Gui, +LastFound
...
if InStr(ft_cors.Event,"OK")
{
  if !A_IsCompiled
  {
    FileRead, fs, %A_LineFile%
    fs:=SubStr(fs,fs~="i)\n[;=]+ Copy The")
  }
  GuiControl, ft_Main:, ft_scr, % ft_cors.Result "`n" fs
  ft_cors.Result:=fs:=""
  return
}
...
ft_WM_LBUTTONDOWN()
{
  ...
  For k,v in ft_C_
    if (v=mid)
    {
      if (k>ft_nW*ft_nH)
      {
        GuiControlGet, i, ft_Capture:, %v%
        GuiControl, ft_Capture:, %v%, % i ? 0:100
      }
      else if (ft_Modify and ft_bg!="")
      {
        c:=ft_cc[k], ft_cc[k]:=c="0" ? "_" : c="_" ? "0" : c
        c:=c="0" ? "White" : c="_" ? "Black" : ft_WindowColor
        Gosub, ft_SetColor
      }
      else
      {
        GuiControl, ft_Capture:, ft_SelCol, % ft_cors[k]
        ft_cors.Color:=ft_cors[k]
      }
      return
    }
}
...
ft_WindowColor:="0xCCDDEE", nW:=ft_nW, nH:=ft_nH
Gui, ft_Capture:Default
...
Gui, +Theme
ListLines, On
Gui, Add, Button, xm+95  w45 gft_Run Section, U
Gui, Add, Button, x+0    wp gft_Run, U3
Gui, Add, Text,   xm+310 yp+6 Section, Color Similarity  0
Gui, Add, Slider
  , x+0 w150 vft_Similar Page1 NoTicks ToolTip Center, 100
Gui, Add, Text,   x+0, 100
Gui, Add, Checkbox, x+15 gft_Run vft_UsePos, UsePos
...
Gui, Add, Button, x+0    wp gft_Run, R3
Gui, Add, Text,   xs     w160 yp, Selected  Color
Gui, Add, Edit,   x+15   w140 vft_SelCol
Gui, Add, Button, x+15   w145 gft_Run, Color2Two
Gui, Add, Button, xm+95  w45 gft_Run, D
Gui, Add, Button, x+0    wp gft_Run, D3
Gui, Add, Text,   xs     w160 yp, Gray Threshold
Gui, Add, Edit,   x+15   w140 vft_Gray
Gui, Add, Button, x+15   w145 gft_Run Default, Gray2Two
Gui, Add, Checkbox, xm   y+21 gft_Run vft_Modify, Modify
Gui, Add, Button, x+5    yp-6 gft_Run, Reset
Gui, Add, Text,   x+20   yp+6, Comment
Gui, Add, Edit,   x+5    w132 vft_Comment
Gui, Add, Button, x+10   yp-6 gft_Run, SplitAdd
...
if !IsObject(ft_cc)
  ft_cc:=[]
left:=right:=up:=down:=k:=0, ft_bg:=""
Loop, % ft_nW*ft_nH {
  ft_cc[++k]:=1, c:=ft_cors[k]
  Gosub, ft_SetColor
}
...
ft_Color2Two:
GuiControlGet, Similar,, ft_Similar
GuiControlGet, r,, ft_SelCol
if r=
{
  MsgBox, 4096, Tip
...
ft_Gray2Two:
gc:=[], k:=0, nW:=ft_nW, nH:=ft_nH
Loop, % nW*nH
  c:=ft_cors[++k], gc[k]:=(((c>>16)&0xFF)*299
    +((c>>8)&0xFF)*587+(c&0xFF)*114)//1000
GuiControl, Focus, ft_Gray
GuiControlGet, Threshold,, ft_Gray
if Threshold=
{
  pp:=[]
  Loop, 256
  ...
  GuiControl,, ft_Gray, %Threshold%
}
ft_color:="*" Threshold, k:=i:=0
Loop, % nW*nH {
  if (ft_cc[++k]="")
    Continue
  if (gc[k]<Threshold+1)
...
GuiControlGet, Comment,, ft_Comment
...
if A_ThisLabel=SplitAdd
{
  ...
  Loop, % w {
    ...
    if v!=
      s.=ft_towz(ft_color,v,SubStr(Comment,1,1))
...
px1:=px-ft_ww+left+(nW-left-right)//2
py1:=py-ft_hh+up+(nH-up-down)//2
...
MouseTip() {
  ...
  x:=NumGet(pt,0,"uint"), y:=NumGet(pt,4,"uint")+NumGet(pt,8,"uint")
